...
contract('RockPaperScissors', (accounts) => {
    ...
    it('Should create game', async () => {
       await contract.createGame(player2, {from: player1, value: 1000});
       let game = await contract.games(0);
       assert(web3.utils.toBN(game.id).toNumber() === 0);
       assert(web3.utils.toBN(game.bet).toNumber() === 1000);
       assert(web3.utils.toBN(game.state).toNumber() === 0);
    });

    it('Should NOT join game if not second player', async () => {
        await contract.createGame(player2, {from: player1, value: 1000});
        await expectRevert(
            contract.joinGame(0, {from: player1, value: 1000}),
    ...
    it('Should NOT join game if not in CREATED state', async () => {
        await contract.createGame(player2, {from: player1, value: 1000});
        await contract.joinGame(0, {from: player2, value: 1000});
        ...
        await expectRevert(
            contract.joinGame(0, {from: player2, value: 1000}),
    ...
    it('Should NOT reveal move if not in state COMMITTED', async () => {
        ...
        await expectRevert(
            ...
            'game must be in COMMITTED state'
        );
    });
    ...
    it('Full game', async () => {
        ...
        assert(web3.utils.toBN(game.id).toNumber() === 0);
        assert(web3.utils.toBN(game.state).toNumber() === 3);
    });
});
